...
class TestQWebTField(TransactionCase):
    ...
    def test_render_t_options(self):
        ...
        text = etree.fromstring(
            self.env['ir.qweb']._render(view1.id)).find('span').text
    ...
    def test_xss_breakout(self):
        ...
        self.assertIn('alert', rendered,
                      "%r doesn't seem to be rendered" % rendered)
...
class TestQWebNS(TransactionCase):
    def test_render_static_xml_with_namespace(self):
        ...
        self.assertEqual(etree.fromstring(self.env['ir.qweb']._render(
            view1.id)), etree.fromstring(expected_result))
    ...
    def test_render_static_xml_with_useless_distributed_namespace(self):
        ...
        self.assertEqual(etree.fromstring(
            self.env['ir.qweb']._render(view1.id)), expected_result)
    ...
    def test_render_static_xml_with_namespace_dynamic(self):
        ...
        values = dict(h1="http://www.example.org/table",
        ...
        self.assertEqual(etree.fromstring(rendering),
    ...
    def test_render_static_xml_with_namespace_dynamic_2(self):
        ...
        values = dict(h1="http://www.example.org/table",
                      h2="http://www.w3.org/TD/html4/", f="http://www.example.org/furniture")
    ...
    def test_render_dynamic_xml_with_namespace_2(self):
        ...
        self.assertEqual(etree.fromstring(self.env['ir.qweb']._render(
            view1.id, {'version_id': 1.0})), expected_result)
    ...
    def test_render_static_xml_with_t_call(self):
        ...
        self.assertEqual(result.count(
            'xmlns:cbc="urn:oasis:names:specification:ubl:schema:xsd:CommonBasicComponents-2"'), 1)
    ...
    def test_render_static_xml_with_extension(self):
        ...
        self.assertEqual(
            etree.fromstring(self.env['ir.qweb'].with_context(
                check_view_ids=[view1.id, view2.id])._render(view1.id)),
            expected_result
        )
...
class TestQWebBasic(TransactionCase):
    def test_compile_expr(self):
        tests = [
            ...
            ("fn(a=11, b=22) or a",                     {
             'a': 1, 'fn': lambda a, b: 0},  1),
            ("fn(a=11, b=22) or a",                     {
             'a': 1, 'fn': lambda a, b: b},  22),
            ("(lambda a: a)(5)",
             {},                             5),
            ("(lambda a: a[0])([5])",
             {},                             5),
        ...
        for expr, q_values, result in tests:
            ...
            self.assertDictEqual(q_values, q_result,
                                 "Should compile: %s" % expr)
    ...
    def test_att_escaping_1(self):
        ...
        result = """
            ...
            """
        values = {'json': json_scriptsafe, 'bibi': dict(
            a='string', b=1), 'toto': "a'b\"c"}
    ...
    # warning for template not found
    ...
    def test_call_error(self):
        ...
        try:
            ...
            error = str(e)
            self.assertIn(
                'External ID not found in the system: base.dummy', error)
    ...
    def test_render_t_call_propagates_t_lang(self):
        ...
        rendered = self.env['ir.qweb'].with_context(
            lang=current_lang)._render(view2.id).strip()
    ...
    def test_render_barcode(self):
        ...
        rendered = self.env['ir.qweb']._render(
            view.id, values={'partner': partner}).strip()
        self.assertRegex(
        ...
        ean_rendered = self.env['ir.qweb']._render(
            view.id, values={'partner': partner}).strip()
        self.assertRegex(
            ean_rendered, r'<div><img style="width:100%;" alt="Barcode" src="data:image/png;base64,\S+"></div>')
        ...
        auto_rendered = self.env['ir.qweb']._render(
            view.id, values={'partner': partner}).strip()
        self.assertRegex(
            auto_rendered, r'<div><img style="width:100%;" alt="Barcode" src="data:image/png;base64,\S+"></div>')
    ...
    def test_render_comment_tail(self):
        ...
        self.assertEqual(self.env['ir.qweb']._render(
            view1.id).strip(), expected)
...
class TestQwebCache(TransactionCase):
    def test_render_xml_cache_base(self):
        ...
        result = etree.fromstring(IrQweb._render(
            view1.id, {'cache_id': 1, 'value': [1, 2, 3]}))
        self.assertEqual(result, expected_result,
                         'First rendering (add in cache)')

        result = etree.fromstring(IrQweb._render(
            view1.id, {'cache_id': 1, 'value': [10, 20, 30]}))
    ...
    def test_render_xml_nocache_use_the_root_values(self):
        ...
        self.assertEqual(etree.fromstring(render), etree.fromstring(
            result), 'rendering 1 (101 != 1: cached t-set should never be applied on root rendering)')
        ...
        """
        self.assertEqual(etree.fromstring(render), etree.fromstring(
            result), 'rendering 2 (102 != 2: cached t-set should never be applied on root rendering)')
        ...
        """
        self.assertEqual(etree.fromstring(render), etree.fromstring(
            result), 'rendering 3 (103 != 3: cached t-set should never be applied on root rendering)')
    ...
    def test_render_xml_nocache_use_the_root_values_and_cached_values(self):
        ...
        """
        self.assertEqual(etree.fromstring(render), etree.fromstring(
            result), 'rendering 1 (1 != 101: new cached values should be add to the root rendering)')
        ...
        """
        self.assertEqual(etree.fromstring(render), etree.fromstring(
            result), 'rendering 2 (102 != 2: cached values should be used)')
        ...
        """
        self.assertEqual(etree.fromstring(render), etree.fromstring(
            result), 'rendering 3 (3 != 103: new cached values should be add to the root rendering)')
    ...
    def test_render_xml_cache_with_t_set_out_of_cache(self):
        ...
        """
        self.assertEqual(etree.fromstring(render), etree.fromstring(
            result), 'rendering 1 (1 != 101: cached t-set should is applied on first rendering)')
        ...
        """
        self.assertEqual(etree.fromstring(render), etree.fromstring(
            result), 'rendering 2 (2 != 102: cached t-set should be applied the template part are rendered every time)')
        ...
        """
        self.assertEqual(etree.fromstring(render), etree.fromstring(
            result), 'rendering 3 (3 != 103: cached t-set should applied because the new cache key is created)')
    ...
    def test_render_xml_cache_with_t_set_in_cache(self):
        ...
        """
        self.assertEqual(etree.fromstring(render),
                         etree.fromstring(result), 'rendering 1')
        ...
        """
        self.assertEqual(etree.fromstring(render),
                         etree.fromstring(result), 'rendering 2')
        ...
        """
        self.assertEqual(etree.fromstring(render),
                         etree.fromstring(result), 'rendering 3')
    ...
    def test_render_xml_cache_with_t_set_wrap_t_cache(self):
        ...
        """
        self.assertEqual(etree.fromstring(render),
                         etree.fromstring(result), 'rendering 4')
    ...
    def test_render_xml_cache_and_inherit_view(self):
        ...
        result = etree.fromstring(
            IrQweb._render(view2.id, {'value': [1, 2, 3]}))
        self.assertEqual(result, expected_result,
    ...
    def test_render_nodb(self):
        ...
        self.assertEqual(html.document_fromstring(rendering),
                         html.document_fromstring(expected))
